...
def pre_process_data(inputs, labels, first_class, second_class):
  """
	...
	:param inputs: an array containing all input images
  :param labels: an array containing all labels of the corresponding images
	...
	"""
  desiredFilter = np.logical_or(labels==first_class, labels==second_class)
  inputs = inputs[desiredFilter]
  inputs = inputs/255
  labels = labels[desiredFilter]
